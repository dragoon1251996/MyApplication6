...
public class ExtensiblePageIndicator extends View implements ViewPager.OnPageChangeListener {
    ...
    private void init(AttributeSet attrs) {
        ...
        int inactiveColor = ContextCompat.getColor(getContext(), R.color.background);
        int activeColor = ContextCompat.getColor(getContext(), R.color.black_semi_transparent);
        int gravity = Gravity.CENTER;

        if (attrs != null) {
            TypedArray a = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.FloatingActionButton, 0, 0);
    ...
    public void initViewPager(ViewPager viewPager) {
        ...
        if(viewPager.getAdapter() == null)
            throw new IllegalStateException("ViewPager doesn't have an AdapterAdd isntance.");
